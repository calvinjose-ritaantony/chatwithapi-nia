...
let chatSocket; // WebSocket connection for chat only
let chatURL = undefined; // URL for chat WebSocket connection
...
document.addEventListener('DOMContentLoaded', function () { 
    ...
    function initChatWebSocket(chatURL) {
        if(chatURL === undefined || chatURL === null || chatURL === ""){
            console.error("Chat URL is not defined. Please provide a valid chat URL.");
            const isStreamingResponse = streamResponsesCheckbox && streamResponsesCheckbox.checked ? true : false;
            chatURL =  isStreamingResponse ? `backend/ws/chat/stream/${current_gpt_id}/${gpt_name}` : `backend/ws/chat/${current_gpt_id}/${gpt_name}`;
        }

        const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        const wsUrl = `${protocol}//${window.location.host}/` + chatURL;
        console.log(wsUrl);
        ...
        chatSocket.onclose = function(event) {
            if (event.wasClean) {
                console.log(`Chat WebSocket connection closed cleanly, code=${event.code}, reason=${event.reason}`);
            } else {
                console.error('Chat WebSocket connection died');
                console.log(event);
    ...
    function handleChatWebSocketMessage(data) {
        if (data.type === 'chat_response') {
            ...
            if (data.follow_up_questions && data.follow_up_questions.length > 0) {
                ...
                data.follow_up_questions.forEach((question) => {
                    ...
                    followUpButton.addEventListener('click', function() {
                        ...
                        if (chatInput) {
                            chatInput.value = followUpButton.dataset.value;
                        } else {
            ...
            if (data.total_tokens) {
                totalTokensDiv.text = `${data.total_tokens}/${maxTokens}`;
                updateTokenProgress(data.total_tokens, maxTokens);
            }
            
            // Hide loading and scroll to bottom after receiving response
            ...
            let aiMessageElement = document.querySelector('.streaming-message');
            
            if (!aiMessageElement) {
                aiMessageElement = document.createElement('div');
                aiMessageElement.classList.add("message", "ai-message", "streaming-message");
                chatHistory.appendChild(aiMessageElement);
            }
            ...
            const aiMessageElement = document.querySelector('.streaming-message');
            if (aiMessageElement) {
                aiMessageElement.classList.remove('streaming-message');
            }
            ...
            console.log("Current event at the backend:");
            console.log(data);

             // Handle regular chat response
            const thinkingMessageElement = document.createElement('div');
            thinkingMessageElement.classList.add("message", "user-message");
            
            var model_response = data.response ? data.response : ''; 
            //model_response = convertMarkDownToHtml(model_response);
            if(""+ model_response.step == undefined || model_response.step == null || model_response.step == ""){
                thinkingMessageElement.innerHTML = model_response;
            }else{
                thinkingMessageElement.innerHTML = model_response.step + "<br>" + model_response.action + "<br>" + model_response.details;
            }
            chatHistory.appendChild(thinkingMessageElement);
        }
    }

    // Send message over WebSocket
    function sendChatWebSocketMessage(chatURL, messageData) {
        if (chatSocket && chatSocket.readyState === WebSocket.OPEN) {
            chatSocket.send(JSON.stringify(messageData));
        } else {
            console.error("Chat WebSocket is not connected");
            handleErrors("WebSocket connection is not established. Please refresh the page.");
            // Attempt to reconnect
            initChatWebSocket(chatURL);
        }
    }
    ...
    window.addEventListener('load', () => {
        userInput.focus();
        //initChatWebSocket(); // Initialize WebSocket connection for chat
    });
                    ...
                    button.addEventListener('click', function() {
                        ...
                        .then(response => {
                            if (response.ok) {
                                ...
                                if(current_gpt_id == undefined || current_gpt_id == "")
                                    initChatWebSocket(chatURL); // Reinitialize WebSocket connection for chat
                            } else {
    ...
    async function loadChatHistory(gpt_id, gpt_name){
        const isStreamingResponse = streamResponsesCheckbox && streamResponsesCheckbox.checked ? true : false;
        chatURL =  isStreamingResponse ? `backend/ws/chat/stream/${current_gpt_id}/${gpt_name}` : `backend/ws/chat/${current_gpt_id}/${gpt_name}`;
        console.log("Chat URL initialized :" + chatURL);
        initChatWebSocket(chatURL); // Initialize WebSocket connection for chat
    ...
    // Function to send a chat message (using WebSockets instead of REST API)
    async function sendMessage(current_gpt_id, gpt_name, user_message, uploadedImage) {
        ...
        try {
            ...
            const isStreamingResponse = streamResponsesCheckbox && streamResponsesCheckbox.checked ? true : false;
            const chatURL =  isStreamingResponse ? `backend/ws/chat/stream/${current_gpt_id}/${gpt_name}` : `backend/ws/chat/${current_gpt_id}/${gpt_name}`;

            // 2. Read the user message and image file if any
            if (uploadedImage) {
                // Convert image to base64 for WebSocket transmission
                const reader = new FileReader();
                reader.onload = function(e) {
                    const base64Image = e.target.result;
                    sendChatWebSocketRequest(chatURL, current_gpt_id, gpt_name, user_message, base64Image);
                };
                reader.readAsDataURL(uploadedImage);
            } else {
                // Send text message only
                sendChatWebSocketRequest(chatURL, current_gpt_id, gpt_name, user_message, null);
            }
        } catch (error) {
            console.error('Error sending message:', error);
            handleErrors('Error sending message: ' + error.message);
            sendLoadingIndicator.style.display = 'none';
            hideLoadingOverlay();
        }
        
        // Don't hide loading here - it will be hidden when response is received
        clearUploadedImageName(); // Clear the uploaded image name
        scrollToUserInputBox(); // Scroll to the bottom of the chat after displaying user message
    }

    function sendChatWebSocketRequest(chatURL, gpt_id, gpt_name, user_message, imageData) {
        ...
        // Send the chat request via WebSocket
        const chatData = {
            type: 'chat_request',
            gpt_id: gpt_id,
            gpt_name: gpt_name,
        ...
        sendChatWebSocketMessage(chatURL, chatData);
    }
    ...
    window.onload = initChatWebSocket;
    window.onbeforeunload = () => {
        ws.close();  // Graceful cleanup
    };
});